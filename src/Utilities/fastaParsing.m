...
% end
if ~exist('gb','var')
    gb = genbankread('NC_008463.1.gb');
end

fasta = {};

for i = 1:length(gb)
    for j = 1:length(gb(i).CDS)
        ...
        for k = 1:hei
            ...
            elseif contains(txt(k,:),'/old_locus_tag=')
                locusTag = erase(txt(k,:),'/old_locus_tag=');
        ...
        fasta{end+1,1} = '>';
        fasta{end,1} = [fasta{end,1},locusTag];
        
        if ~isempty(gene)
            fasta{end,1} = [fasta{end,1},'[gene=',gene,']'];
        end
        if ~isempty(product)
            fasta{end,1} = [fasta{end,1},'[product=',product,']'];
        end
        if ~isempty(proteinId)
            fasta{end,1} = [fasta{end,1},'[protein_id=',proteinId,']'];
        end
        if ~isempty(geneId)
            fasta{end,1} = [fasta{end,1},'[gene_id=',geneId,']'];
        end
...
writecell(fasta,'NC_008463.1.txt','QuoteStrings',false);
